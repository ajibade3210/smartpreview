  ...
  return (
    <div className="flex h-screen flex-col bg-background overflow-hidden">
      ...
      <main className="relative flex flex-1 overflow-hidden">
        {/* Render a dynamic split-pane workspace when both Editor and Preview are toggled */}
        {viewMode === "both" ? (
          <PanelGroup direction="horizontal" className="flex flex-1 w-full h-full">
            {/* Left Column: Code Editor & Variables Panel */}
            <Panel defaultSize={50} minSize={20} className="h-full flex flex-col">
            ...
            {/* Interactive Resizing Divider: Provides visual hover transitions and a drag handle */}
            <PanelResizeHandle className="w-1.5 bg-border hover:bg-primary/60 hover:w-2 active:bg-primary/80 transition-all cursor-col-resize relative z-20 flex items-center justify-center group">
              {/* Central resizer grab bar visual indicator */}
              <div className="h-8 w-1 rounded bg-muted-foreground/30 group-hover:bg-primary transition-colors" />
            </PanelResizeHandle>
            
            {/* Right Column: Live Output Render Frame & Developer Console */}
            <Panel defaultSize={50} minSize={20} className="h-full flex flex-col">
              <div className="flex h-full flex-col">
